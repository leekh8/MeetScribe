...
작은 모델은 빈 칸을 만나면 그럴듯한 값을 채운다. 실측에서 전사본에 한 번도 안 나오는
"개발자"를 담당자로 5건 전부에 넣었다. 프롬프트로 금지해도 막히지 않는다.
그래서 액션아이템의 **담당자와 기한은 전사본에 실제로 있는 문자열일 때만 남긴다.**
요약문과 달리 이 둘은 인용이라 원문 대조가 된다. 비는 것보다 틀린 것이 나쁘다.
...
```json
{
  "홍길동": ["홍길동 주임", "길동 주임", "홍길동"],
  "김영희": ["김영희 차장", "영희 팀장", "팀장님"]
}
```
